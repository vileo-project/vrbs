...
BEGIN
	IF NEW.posts != OLD.posts THEN
		...
		WHERE
			...
			ls0.id_referrer = ll1.id_reflink
		;
		UPDATE
			smf_vrcs_stat ls0,
			smf_vrcs_link ll1,
		...
		WHERE
			OLD.id_member = ls0.id_member AND
			ls0.id_referrer = ll1.id_reflink AND
			ll1.id_member = ls1.id_member AND
			ls1.id_referrer = ll2.id_reflink AND
		...
		WHERE
			...
			ls2.id_referrer = ll3.id_reflink AND
...
CREATE PROCEDURE smf_vrcs_addmembers (IN p_id_member MEDIUMINT UNSIGNED, IN p_id_referrer INT UNSIGNED)
BEGIN
	IF p_id_referrer NOT IN (SELECT id_reflink FROM smf_vrcs_link) THEN
		SET p_id_referrer = 0;
	END IF;
	INSERT INTO smf_vrcs_stat (id_member, id_referrer) VALUES (p_id_member, p_id_referrer);
	IF p_id_referrer != 0 THEN
		UPDATE smf_vrcs_link SET counter_l1 = counter_l1 + 1 WHERE id_reflink = p_id_referrer;
		UPDATE
			smf_vrcs_link ll1,
			smf_vrcs_stat ls1,
			smf_vrcs_link ll2,
		...
		WHERE
			p_id_referrer = ll1.id_reflink AND
			ll1.id_member = ls1.id_member AND
			ls1.id_referrer = ll2.id_reflink AND
			ll2.id_member = ls2.id_member
		;
